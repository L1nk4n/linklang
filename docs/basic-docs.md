...
These are docs for people who might want to contribute. They explain how to create a pull request and how to contribute to this project. I've tried to keep this as straightforward as possible — please contact me if there are improvements to make or syntax errors to fix.
...
│   ├── lexer/
│   │   ├── token.h           — Defines TokenType enum (all token kinds) and
│   │   │                       Token struct {value, type}
│   │   ├── token.cpp         — (empty)
│   │   ├── lexer.h           — (empty)
...
│   │                           (statements)
│   ├── semantic/
│   │   ├── semantic.h        — Defines AST node types (Stmt/Expr), Type enum,
│   │   │                       Scope, Symbol, SemanticAnalyzer
...
| Function                       | What it does                                                                                                                                                          |
|--------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `INIT_RESERVED_IDENTIFIER()`   | Populates the `reservedIdent` map with all keywords and built-in function names, and their `TokenType`                                                                |
...
| `isStringLiteral(str)`         | Returns true if string starts and ends with `"`                                                                                                                       |
| `isLong(str)`                  | Returns true if string is a valid `long long` integer                                                                                                                 |
...
| `isAlpha(str)`                 | Returns true if string is a valid identifier (starts with letter/underscore, then alphanumeric/underscore)                                                            |
...
| `isTypeToken(type)`          | Returns true if token is a type keyword (`String`, `Bool`, `Double`, `Float`, `Long`, `I8`–`I128`, `Ui8`–`Ui128`) |
...
| `expression()`               | Parses binary/comparison/logical expressions (left-to-right)                                                  |
...
| Function                               | What it does                                                                                               |
|----------------------------------------|------------------------------------------------------------------------------------------------------------|
| `Scope::lookup(name)`                  | Walks the scope chain upward to find a symbol by name                                                      |
| `Scope::declare(name, sym)`            | Declares a symbol in the current scope. Returns false if already exists                                    |
| `FunctionDecl::analyze(ctx)`           | Registers function in scope, creates child scope for params + body, analyzes body                          |
...
| `ReturnStmt::analyze(ctx)`             | Checks return is inside a function and that its expression type matches the declared return type                                                                            |
